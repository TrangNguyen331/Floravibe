...
<<<<<<< Updated upstream
  const applyCoupon = () => {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
      console.log("applied");
    }, 1300); // Example: 1 second delay, adjust as needed
  };
  const placeOrder = async () => {
    if (token) {
      ...
      const voucherDetail = selectedVoucher
        ? {
  ...
  const updateVoucherQty = async (voucherId, body) => {
    try {
      await axiosInstance.put("/api/v1/vouchers/" + voucherId, body);
    } catch (error) {
      // Xử lý khi gặp lỗi khi cập nhật voucher
...
=======
    ...
    return ( <
            ...
            Checkout <
            /BreadcrumbsItem> <
            LayoutOne headerTop = "visible" > { /* breadcrumb */ } <
            Breadcrumb / >
            <
            div className = "container mt-5" >
            <
            ...
            div className = "checkout-area pt-95 pb-100" >
            <
            div className = "container" > { /* {cartItems && cartItems.length >= 1 ? ( */ } <
            div className = "row" >
            <
            div className = "col-lg-7" >
            <
            ...
            ul > {
                cartItems.map((cartItem, key) => {
                    ...
                    return ( <
                        li key = { key } >
                        <
                        span className = "order-middle-left" > { cartItem.name }
                        X { cartItem.quantity } <
                        /span>{" "} <
                        span className = "order-price" > {
                            /* {discountedPrice !== null
